...
Let's note that researchers test their algorithms using different datasets (a new ImageNet dataset is released as a new challenge with different images each year). Thus the cited accuracies cannot be directly compared *per se*.


## The ImageNet challenge
The ImageNet database is the outcome of a collaboration between Stanford University and Princeton University, and has become a reference in the field of computer vision. It contains around 14 millions images originally labeled with Synsets[^1] of the WordNet lexicon tree. The original challenge
...
Inspired from the previous work, they have reduced the number of available functions and the combination operators. They have removed the Reinforcement Learning part to create a progressive search. They first start by construct all the possible simple structures with a single function. Each one of them is stacked with an operator to another structure and a learned function predicts their score to quantify the profitability of learning it. These scores are ordered and the K best ones are selected to be stacked with another structure. The process is iterative until the maximum number of block is reached.

The PNAS model have achieved the same performances than the NAS model developed by  [B. Zoph et al. (2017)](https://arxiv.org/abs/1707.07012) while reducing by a factor of 5 the training time. This method is promising for deep learning because new intuitive architectures are difficult to find.
